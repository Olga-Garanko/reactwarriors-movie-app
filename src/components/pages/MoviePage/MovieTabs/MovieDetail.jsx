      ...
      <div className="mt-4">
        <table className="table">
          ...
          <tr>
            <th>Страна</th>
            <td>
              { movieDetail.production_countries && movieDetail.production_countries.length && movieDetail.production_countries.map((country, idx) => <div key={idx}>{country.name} </div>)}
          ...
          <tr>
            <th>Компания</th>
            <td>
              { movieDetail.production_companies && movieDetail.production_companies.length && movieDetail.production_companies.map(company => <div key={company.id}><span className="badge badge-info mr-4">{company.name}</span></div>)}
